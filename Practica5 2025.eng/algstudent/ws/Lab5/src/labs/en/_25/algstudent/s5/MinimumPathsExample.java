...
public class MinimumPathsExample {
	...
	static void floyd(int[][] weights, int[][] costs, int[][] p) {
		...
		for(int node= 0; node < v.length; node++) {
			for(int pivot = 0; pivot < v.length; pivot++) {
				for(int dest = 0; dest < v.length; dest++) {
					if(node != dest)
						{if(weights[node][dest] > weights[node][pivot] + weights[pivot][dest])
							{
								costs[node][dest] = weights[node][pivot] + weights[pivot][dest];
								p[node][dest] = pivot;
							}
						else
	...
	static void minimumPath(String[] v, int[][] weights, int[][] costs, int[][] steps, int source, int target) {
		...
		if(pivot != -1)
			minimumPath(v, weights,  costs,  steps,pivot,  target);
		else
