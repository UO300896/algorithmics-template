...
public abstract class Node implements Comparable<Node> {
    ...
    public int totalWeight; //STUDENT: attribute to consider the total weight so far. later it is added to the heuristic value
	...
	public int compareTo(Node node) { //BRANCHING METHOD
		int totalValue = Math.abs(heuristicValue); //STUDENT: Added abs value to avoid priorizing negative numbers but nearest to 0 numbers.
		int totalValueToBeCompared = Math.abs(node.getHeuristicValue()); //STUDENT: Same reason
